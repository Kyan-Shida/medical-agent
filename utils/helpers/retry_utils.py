...
from utils.exceptions.exception_utils import LLMCallError, LLMTimeoutError
...
def retry_with_backoff(
    ...
    带指数退避的重试装饰�?
    Args:
        max_retries: 最大重试次�?        base_delay: 基础延迟时间（秒�?        max_delay: 最大延迟时间（秒）
        exponential: 是否使用指数退�?        jitter: 是否添加随机抖动
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    # 限制最大延�?                    delay = min(delay, max_delay)
...
class TenacityRetry:
    ...
    def create(
        ...
        创建重试装饰�?
        Args:
            max_retries: 最大重试次�?            base_delay: 基础延迟
            max_delay: 最大延�?            exceptions: 重试异常类型
